...
namespace Validation.Validation
{
    public class GroupItemPriceValidator : IGroupItemPriceValidator
    {
        ...
        {
            IList<PriceMutation> priceMutations = _priceMutationService.GetObjectsByIsActive(true, groupItemPrice.ItemId, groupItemPrice.GroupId, 0);
        ...
        {
            if (groupItemPrice.GroupId != oldgroupItemPrice.GroupId)
            {
                groupItemPrice.Errors.Add("GroupId", "Tidak boleh beda");
            }
            return groupItemPrice;
        }
        ...
        public GroupItemPrice VIsValidGroup(GroupItemPrice groupItemPrice, IGroupService _groupService)
        {
            Group group = _groupService.GetObjectById(groupItemPrice.GroupId);
            if (group == null)
            {
                groupItemPrice.Errors.Add("GroupId", "Tidak valid");
            }
            return groupItemPrice;
        }
        ...
        public GroupItemPrice VCreateObject(GroupItemPrice groupItemPrice, IGroupItemPriceService _groupItemPriceService, IGroupService _groupService, IItemService _itemService)
        {
            ...
            if (!isValid(groupItemPrice)) { return groupItemPrice; }
            VIsValidGroup(groupItemPrice, _groupService);
            if (!isValid(groupItemPrice)) { return groupItemPrice; }
        ...
        public bool ValidCreateObject(GroupItemPrice groupItemPrice, IGroupItemPriceService _groupItemPriceService, IGroupService _groupService, IItemService _itemService)
        {
            VCreateObject(groupItemPrice, _groupItemPriceService, _groupService, _itemService);
