$(document).ready(function () {
    ...
    function ReloadGrid() {
        $("#list").setGridParam({ url: base_url + 'BlendingWorkOrder/GetList', postData: { filters: null }, page: 'first' }).trigger("reloadGrid");
    }
    ...
    $("#list").jqGrid({
        ...
        colModel: [
                  ...
                  { name: 'blendingdate', index: 'blendingdate', search: false, width: 100, align: "center", formatter: 'date', formatoptions: { srcformat: 'Y-m-d', newformat: 'm/d/Y' } },
    ...
    $('#btn_add_new').click(function () {
        ClearData();
        clearForm('#frm');
        $('#BlendingDate').datebox('setValue', $.datepicker.formatDate('mm/dd/yy', new Date()));
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            $.ajax({
                dataType: "json",
                url: base_url + "BlendingWorkOrder/GetInfo?Id=" + id,
                success: function (result) {
                    if (result.Id == null) {
                        ...
                        if (JSON.stringify(result.Errors) != '{}') {
                            ...
                            $('#BlendingDate').datebox('setValue', dateEnt(result.BlendingDate));
                            $('#form_div').dialog('open');
                        }
                    }
                }
    ...
    $('#btn_unconfirm').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to unconfirm record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "BlendingWorkOrder/Unconfirm",
                        type: "POST",
    ...
    $('#delete_confirm_btn_submit').click(function () {
        ...
        $.ajax({
            url: base_url + "BlendingWorkOrder/Delete",
            type: "POST",
            contentType: "application/json",
            data: JSON.stringify({
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'BlendingWorkOrder/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'BlendingWorkOrder/Insert';
        }
    ...
    $("#lookup_table_blendingrecipe").jqGrid('navGrid', '#lookup_toolbar_blendingrecipe', { del: false, add: false, edit: false, search: true })
