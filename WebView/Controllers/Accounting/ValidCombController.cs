...
namespace WebView.Controllers
{
    public class ValidCombController : Controller
    {
        ...
        public dynamic GetList(string _search, long nd, int rows, int? page, string sidx, string sord, string filters = "")
        {
            ...
            // Get Data
            var q = _validCombService.GetQueryable().Include("Account").Include("Closing").Where(x => x.Account.IsLeaf && x.Amount > 0);
        ...
        public dynamic GetListByDate(string _search, long nd, int rows, int? page, string sidx, string sord, int? ClosingId, string filters = "")
        {
            ...
            // Get Data
            var q = _validCombService.GetQueryable().Include("Account").Include("Closing").Where(x => x.Closing.IsClosed && x.Account.IsLeaf && x.Amount > 0);
