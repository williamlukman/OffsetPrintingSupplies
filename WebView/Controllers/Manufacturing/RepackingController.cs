...
namespace WebView.Controllers
{
    ...
    {
        private readonly static log4net.ILog LOG = log4net.LogManager.GetLogger("RepackingController");
        ...
        public dynamic GetList(string _search, long nd, int rows, int? page, string sidx, string sord, string filters = "")
        {
            ...
            // Get Data
            var q = _repackingService.GetQueryable().Where(x => !x.IsDeleted);
        ...
        public dynamic GetListNonConfirmed(string _search, long nd, int rows, int? page, string sidx, string sord, string filters = "")
        {
            ...
            // Get Data
            var q = _repackingService.GetQueryable().Where(x => !x.IsDeleted && !x.IsConfirmed);
        ...
        {
            Repacking model = new Repacking();
            try
            {
                model = _repackingService.GetObjectById(Id);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public dynamic Insert(Repacking model)
        {
            try
            {
                model = _repackingService.CreateObject(model, _blendingRecipeService, _warehouseService);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public dynamic Update(Repacking model)
        {
            try
            {
                var data = _repackingService.GetObjectById(model.Id);
                data.BlendingRecipeId = model.BlendingRecipeId;
                data.WarehouseId = model.WarehouseId;
                data.Code = model.Code;
                data.Description = model.Description;
                data.RepackingDate = model.RepackingDate;
                model = _repackingService.UpdateObject(data, _blendingRecipeService, _warehouseService);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public dynamic Delete(Repacking model)
        {
            try
            {
                var data = _repackingService.GetObjectById(model.Id);
                model = _repackingService.SoftDeleteObject(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public dynamic Confirm(Repacking model)
        {
            try
            {
                var data = _repackingService.GetObjectById(model.Id);
                model = _repackingService.ConfirmObject(data, model.ConfirmationDate.Value, _blendingRecipeService, _blendingRecipeDetailService,
        ...
        [HttpPost]
        public dynamic UnConfirm(Repacking model)
        {
            try
            {

                var data = _repackingService.GetObjectById(model.Id);
                model = _repackingService.UnconfirmObject(data, _blendingRecipeService, _blendingRecipeDetailService, 
                            _stockMutationService, _blanketService, _itemService, _itemTypeService, _warehouseItemService,
                            _generalLedgerJournalService, _accountService, _closingService);
            }
            catch (Exception ex)
